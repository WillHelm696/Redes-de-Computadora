...
void analizar_tramas(string tramas){
    ...
    while (i <= tramas.length()){
        ...
        int suma = 0 ; // Suma calculara el checksun

        string byte_bandera = tramas.substr(i,2);
        if (byte_bandera == "7E"){
            string byte_longitud = tramas.substr(i+2,4);
            string sub_trama ; // Sub_Trama Selecsiona se encadena las tramas despues de la longitud
            ...
            while ( j < ( i + 6 + longitud * 2 ) ){
                ...
                if (byte == "7E"){
                    string byte_escape = tramas.substr(j-2,2); // SI se envontro un Byte 7E verifica si antes hay un 7D
                    if (byte_escape != "7D"){ 
                ...
                suma += stoi(byte, nullptr, 16); // Se lleva a cabo una suma de el byte para verificar el checksum pasando de caracter a base 16
            ...
            int checksum = stoi(byte_checksum, nullptr, 16); // Convierte el checksum pasando de caracter a base 16
            ...
            if (tramas.substr(i+ 8 + sub_trama.length() ,2)!="7E" or sub_trama.length() != longitud*2 ) {
                ...
                SI tramas.substr(i+ 8 + sub_trama.length() ,2)!="7E"  da falso es mas largo la trama 
                SI sub_trama.length() != longitud*2 da falso es mas corto la trama
                */ 
                byte_checksum = ""; // Si no es la longitud correcta el checksum estara mal por lo que eatara vacio
                tramas_longitud_incorrecta ++ ; // Se lleva a cabo un acuenta de longitud incorectas
            ...
            cout <<"/ byte SALTADO:" << tramas.substr(i-2,2)<< endl;
            i += 2 ;
        }
    }
